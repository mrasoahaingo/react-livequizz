...
describe('application logic', () => {
    describe('player', () => {
        it('add new player', () => {
            const state = fromJS({
                players: [
                    { id: 1, name: 'Bryan', score: 0, isConnected: true, isReady: false }
                ]
            });
            ...
            expect(nextState).to.equal(fromJS({
                players: [
                    { id: 1, name: 'Bryan', score: 0, isConnected: true, isReady: false },
                    { id: 2, name: 'Sandy', score: 0, isConnected: true, isReady: false, isOut: true }
                ]
            }));
        });
        ...
        it('add existing player', () => {
            const state = fromJS({
                players: [
                    { id: 1, name: 'Bryan', score: 0, isConnected: true },
                    { id: 2, name: 'Sandy', score: 0, isConnected: false }
                ]
            });
            const nextState = Core.addPlayer(state, 'Sandy', 2);
            expect(nextState).to.equal(fromJS({
                players: [
                    { id: 1, name: 'Bryan', score: 0, isConnected: true },
                    { id: 2, name: 'Sandy', score: 0, isConnected: true }
                ]
            }));
        });
        ...
        it('disconnectPlayer', () => {
            ...
            expect(nextState).to.equal(fromJS({
                players: [
                    { id: 1, name: 'Bryan', score: 0, isConnected: false },
                    { id: 2, name: 'Sandy', score: 0, isConnected: true }
                ]
            }));
        });
    ...
    describe('next', () => {
        ...
        it('have a winner', () => {
            const state = fromJS({
                players: [
                    { id: 1, name: 'Bryan', score: 3 },
                    { id: 2, name: 'Sandy', score: 1 }
                ],
                entries: []
            });
            const nextState = Core.next(state);
    ...
    describe('responding', () => {
        ...
        it('give the wrong response', () => {
            ...
            expect(nextState.filter(keyIn('players', 'quizz'))).to.equal(fromJS({
                players: [
                    { id: 1, name: 'Bryan', isOut: true }
                ],
                quizz: { question: 'Where is Bryan', response: 'In the kitchen' }
